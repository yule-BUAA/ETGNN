...
def generate_baskets(transaction_df: pd.DataFrame, out_sequence_path: str, items_map_dict_path: str, users_map_dict_path: str):
    ...
    for user_id, user_df in tqdm(transaction_df.groupby(['customer_id'])):
        baskets_dict = {}
        for day, trans in user_df.groupby(['day']):  # select by user and day
            product_index_list = list(set(trans['product_id'].tolist()))
            baskets_dict[(list(set(trans['week'].tolist()))[0], day)] = product_index_list   # {(week, day): [item_1_id, item_2_id], (week, day): [], ...}

        baskets_list = sorted(baskets_dict.items(), key=lambda item: item[0][1], reverse=False)  # sorted by day
    ...
    print(f'select date in the first {select_days} days')
...
if __name__ == "__main__":
    data_path = "../original_data/dunnhumby-Carbo/dunnhumby-Carbo/dh_transactions.csv"
    ...
    transaction_df = get_frequent_items(transaction_df, frequency_rate=frequency_rate, key='product_id')
